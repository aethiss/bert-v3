...
export function Distribution({ route, onNavigate, session }: ClientDistributionProps) {
  ...
  const handleSearch = async (): Promise<void> => {
    if (!session) {
      toast.error('Disconnected', {
        description: 'Connect to the host before searching members.'
      });
      return;
    }
  ...
  const handleConfirm = async (): Promise<void> => {
    if (!session) {
      toast.error('Disconnected', {
        description: 'Connect to the host before confirming distribution.'
      });
      return;
    }
    ...
    try {
      ...
      await saveDistributionOnLocalServer(session, {
        subOperator: session.alias,
        cycleCode: selectedCycleCode,
        memberId: selectedMemberId
      });

      await saveClientDistributionHistory({
        alias: session.alias,
        host: session.host,
        memberId: selectedMemberId,
        familyUniqueCode: detail.household.familyUniqueCode,
        cycleCode: selectedCycleCode,
        cycleName: selectedCycle?.cycleName ?? `Cycle ${selectedCycleCode}`,
        collectedBy: selectedMember.fullName
      });
      ...
      if (printSettings.disabled) {
        toast.success('Saved', {
          description: 'Distribution confirmed on host server.'
        });
        onNavigate({
          ...route,
          section: 'distribution',
